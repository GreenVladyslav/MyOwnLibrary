...
$.prototype.modal = function(created) { // №С)create -создано ли это окно программно
        ...
        closeElements.forEach(elem => {
            $(elem).click(() => {
                ...
                setTimeout(() => {
                    ...
                    if (created) { //№С) если это модальное окно было создано прогрмано то мы его удалим со страницы
...
$.prototype.createModal = function({text, btns} = {}) { // [3 вариант]
    for (let i = 0; i < this.length; i++) {
        let modal = document.createElement('div');
        modal.classList.add('modal');
        modal.setAttribute('id', this[i].getAttribute('data-target').slice(1)); // убрали решокту в начале
